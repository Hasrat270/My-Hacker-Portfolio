    ...
    <div className="flex flex-col gap-8 max-w-2xl">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex flex-col gap-2">
          <p className="font-mono text-[10px] text-[#00ff41]/30 tracking-widest">profile pic</p>
          <label className="font-mono text-xs text-[#00ff41] border border-[#00ff41]/40 px-3 py-1.5 rounded hover:bg-[#00ff41]/10 transition-all duration-200 cursor-pointer">
            ...
            <input
              ...
              accept="image/*"
              className="hidden"
              onChange={e => e.target.files[0] && picMutation.mutate(e.target.files[0])}
            />
          </label>
          {picMutation.isSuccess && (
            <p className="font-mono text-[10px] text-[#00ff41]/50">✓ uploaded</p>
          )}
        </div>
      </div>
      ...
      <div className="border border-[#00ff41]/20 rounded p-4 bg-[#00ff41]/2 flex items-center justify-between gap-4">
        <div>
          <p className="font-mono text-[10px] text-[#00ff41]/30 tracking-widest mb-1">resume.pdf</p>
          {profile?.resume
            ? <a href={profile.resume} target="_blank" rel="noreferrer" className="font-mono text-xs text-[#00ff41]/60 hover:text-[#00ff41] transition-colors">view current resume</a>
            : <p className="font-mono text-xs text-[#00ff41]/30">no resume uploaded</p>
          }
        </div>
      ...
      <div className="flex flex-col gap-4">
        {[
          ...
          { key: 'designation', label: 'designation', placeholder: 'Penetration Tester'   },
        ...
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[10px] text-[#00ff41]/40 tracking-widest">bio</label>
          <textarea
            name="bio"
            value={form.bio}
            onChange={handleChange}
